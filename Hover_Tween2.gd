...
func apply_to_all_fullart2(root: Node) -> void:
	for child in root.get_children():
		# if this child is a TextureButton and has a Fullart
		if child is TextureButton and child.has_node("Fullart2"):
			var fullart: TextureRect = child.get_node("Fullart2")
			add_hover_tween(child, fullart)

		# recurse down into its children too
		apply_to_all_fullart2(child)
